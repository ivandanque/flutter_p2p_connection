...
class P2pTransportClient with FileRequestServerMixin {
  ...
  /// A stream of received text messages with sender IDs.
  Stream<({String senderId, String text})> get receivedTextWithSenderStream =>
      _receivedTextWithSenderController.stream;
  ...
  Future<void> connect() async {
    ...
    final bool fileServerStarted = await _startFileServer();
    ...
    _socketSubscription = _socket!.listen(
      (data) async {
        try {
          ...
          final P2pClientInfo senderInfo = _clientList.firstWhere(
          ...
          switch (message.type) {
            case P2pMessageType.clientList:
              ...
              ); // Self is included in client list, remove it
              debugPrint(
      ...
      onDone: () async {
        ...
        if (_isManuallyDisconnecting) {
          await _cleanupConnectionState(
            stopFileServer: true,
          ); // Manual disconnect stops server
    ...
    if (fileInfo != null) {
      if (fileInfo.receiverIds.contains(progressUpdate.receiverId)) {
        fileInfo.updateProgress(
          progressUpdate.receiverId,
          progressUpdate.bytesDownloaded,
          progressUpdate.fileState,
        );
        debugPrint(
  ...
  Future<P2pFileInfo?> shareFile(
    File file, {
    required String actualSenderIp,
    List<P2pClientInfo>? recipients,
  }) async {
    if (!isConnected) {
      debugPrint(
    ...
    final bool sent = await send(message);
  ...
  Future<void> _sendProgressUpdateToServer(
    ...
    final P2pClientInfo targetClient = _clientList.firstWhere(
      (c) => c.id == originalSenderId,
      orElse: () {
